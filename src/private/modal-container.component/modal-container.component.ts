...
import * as modalConfiguration from '../../modal.configuration';
import {
	KxModalComponent,
	KxModalComponentRef
...
export interface IKxModalStylingAnimationWithFactory extends modalConfiguration.IKxModalStylingAnimation {
...
export class KxModalContainerComponent implements IKxModalContainerCreator, OnDestroy, OnInit {
	...
	constructor(
		...
		@Inject(modalConfiguration.MODAL_STYLING_TOKEN) modalStyling: modalConfiguration.IKxModalStyling
	) {
		this.modalContainerService.registerContainerComponent(this);

		this.bodyStyling = this.createModalStylingPart(modalStyling.body);
		this.modalBackdropStyling = this.createModalStylingPart(modalStyling.modalBackdrop);
	...
	private attachComponentStyles<T>(
		...
		this.applyComponentClasses(element, `${this.modalStyling.class} ${configurationStyling.class}`.trim());
	...
	private updateContainer() {
		...
		if (modalComponentCount === 0) {
			this.renderer.removeClass(document.body, this.bodyStyling.class);
		} else {
			this.renderer.addClass(document.body, this.bodyStyling.class);
		}
	}
